...
if ($conn) { 
    $query = "SELECT clientname, transaction_id, approve_at, status, form_type 
              FROM appointments 
              WHERE ((status = 'Declined' AND archived = 1 AND paid = 0) 
...
if (isset($_GET['generate_pdf'])) {
    ...
    if ($result->num_rows > 0) {
        while ($client = $result->fetch_assoc()) {
            ...
            $client['approve_date'] = $client['approve_at']; // Ensure the key matches the column name
            unset($client['approve_at']); // Remove the old key
            $client['form_type'] = formatFormType($client['form_type']); // Format the form_type
            $data[] = $client;
        }
    }
    $columns = ['Client Name', 'Transaction ID', 'Approve Date', 'Form Type', 'Status'];
    $outputFileName = 'Client_Report_' . date('Y-m-d_H-i-s') . '.pdf';
  
    generatePDF($data, 'Client Report', $columns, $outputFileName, false); 
    exit;
}
